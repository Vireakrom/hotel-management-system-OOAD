    ...
    public class StrategyPatternTests
    {
        ...
        private static bool TestCashPaymentStrategy()
        {
            try
            {
                ...
                // Get valid user ID for payment processing
                UserRepository userRepo = new UserRepository();
                var users = userRepo.GetAll();
                if (users == null || users.Count == 0)
                {
                    Console.WriteLine("Test 1: No users found in database");
                    return false;
                }
                int validUserId = users[0].UserId;
                
                // Process payment
                Payment payment = cashStrategy.ProcessPayment(testInvoice, 100.00m, validUserId);
        ...
        private static bool TestCreditCardPaymentStrategy()
        {
            try
            {
                ...
                if (users == null || users.Count == 0)
                {
                    Console.WriteLine("Test 2: No users found in database");
                    return false;
                }
                int validUserId = users[0].UserId;
                
                // Process payment
                Payment payment = ccStrategy.ProcessPayment(testInvoice, 110.00m, validUserId);
        ...
        private static bool TestPartialPayment()
        {
            try
            {
                ...
                if (users == null || users.Count == 0)
                {
                    Console.WriteLine("Test 4: No users found in database");
                    return false;
                }
                int validUserId = users[0].UserId;
                ...
                // Pay $100 (partial)
                Payment payment1 = context.ExecutePayment(testInvoice, 100.00m, validUserId);
        ...
        private static bool TestFullPaymentWorkflow()
        {
            try
            {
                ...
                // Pay with cash
                decimal change;
                Payment cashPayment = paymentContext.ProcessCashPayment(invoice, 250.00m, validUserId, out change);
        ...
        private static Invoice CreateTestInvoice()
        {
            try
            {
                ...
                // Get first available guest, room, and user from database
                ...
                if (users == null || users.Count == 0)
                {
                    throw new Exception("No users found in database for testing");
                }
